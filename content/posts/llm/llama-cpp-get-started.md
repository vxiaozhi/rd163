...
date = "2025-03-08"
lastmod = "2025-03-08"
subtitle = "从源码编译、GGUF 量化到本地部署的完整上手指南"
description = "介绍 llama.cpp 的安装编译、GGUF 模型量化流程、常用命令行用法（llama-cli/llama-server/llama-bench），以及在 Apple Silicon 上部署 GGUF 模型的实践经验。"
author = "小智晖"
authors = ["小智晖"]
categories = ["llm"]
tags = ["llm", "llama.cpp", "GGUF", "模型量化", "本地部署", "DeepSeek"]
keywords = ["llama.cpp", "GGUF", "模型量化", "llama-server", "本地部署", "DeepSeek"]
toc = true
draft = false
+++
...
```bash
python3 llama.cpp/convert.py [原始模型目录] --outfile [输出路径]/phi-2-f16.gguf
```
...
```bash
./llama.cpp/quantize [输入GGUF文件] [输出量化文件] [量化类型]
# 示例：./llama.cpp/quantize phi-2-f16.gguf phi-2-Q4_K_M.gguf Q4_K_M
```
...
```bash
./llama.cpp/main -m phi-2-Q4_K_M.gguf -p "测试文本" -n 128
```
